...
sudo tee /etc/nginx/sites-available/ci-cd-test > /dev/null << 'EOF'
server {
    listen 80;
...
$nginxScript | Out-File -FilePath "temp_nginx_install.sh" -Encoding UTF8 -NoNewline

# Копируем скрипт в WSL и выполняем
wsl -d Ubuntu-22.04 -e bash -c "cp '/mnt/c/Users/Administrator/prodject/temp_nginx_install.sh' /tmp/"
wsl -d Ubuntu-22.04 -e bash -c "chmod +x /tmp/temp_nginx_install.sh"
wsl -d Ubuntu-22.04 -e bash -c "/tmp/temp_nginx_install.sh"

# Удаляем временный файл
Remove-Item "temp_nginx_install.sh" -Force
