# Гибридный CI/CD скрипт - работает с nginx в WSL, готов к Docker
...
Write-Host "=== Гибридный CI/CD (WSL + готов к Docker) ===" -ForegroundColor Green
...
try {
    wsl bash -c "echo 'ubuntu' | sudo -S cp -r /mnt/c/Users/Administrator/prodject/nginx-site/* /var/www/local-test/"
...
if ($gitStatus) {
    ...
    # Push с обходом хуков
    try {
        git push --no-verify
        Write-Host "✓ Изменения отправлены в репозиторий (хуки пропущены)" -ForegroundColor Green
    } catch {
...
Write-Host "`n=== Гибридный CI/CD процесс завершен успешно ===" -ForegroundColor Green
...
Write-Host "`n🎉 Все готово! Можете открыть браузер и проверить сайт." -ForegroundColor Green
Write-Host "💡 Для перехода на Docker: установите Docker Desktop и используйте ci-cd-docker.ps1" -ForegroundColor Yellow 